 ...
 * Opsiyonel crop parametreleri: { cropX, cropY, cropScale } — kullanıcının
 * önizlemede seçtiği konum/zoom'a göre kırpar (aynı mantık optimizeProfileCover ile).
 * Crop parametresi verilmezse (veya varsayılan değerlerdeyse) merkezi kare kırpma yapılır.
 */
export async function optimizeAvatar(inputBuffer, outputPath, cropOptions = {}) {
    ...
    // Crop yoksa (kullanıcı hiçbir ayar yapmadıysa), zaten WebP ve uygunsa doğrudan kopyala
    ...
    // Kullanıcının belirlediği konum/zoom'a göre kırp (optimizeProfileCover ile aynı mantık, 200x200 için)
    const meta = await sharp(inputBuffer).metadata();
    ...
    let extractLeft = Math.round(scaledW / 2 - 100 + cropX * 2);
    let extractTop = Math.round(scaledH / 2 - 100 + cropY * 2);
