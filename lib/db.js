...
function migrateDatabase(db) {
  try {
    ...
    // Feature: User Ratings — kullanıcılar seriye 1-5 yıldız (puan) verebilir
    db.exec(`
    ...
    // Migration: eski 1-10 puan sistemi → 1-5 yıldız sistemine geçiş
    // CHECK(rating <= 10) içeren eski tablo varsa yeniden oluştur ve mevcut puanları 2'ye bölerek dönüştür
    try {
      const urTableSql = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='user_ratings'").get();
      if (urTableSql?.sql && /rating\s*<=\s*10/.test(urTableSql.sql)) {
        db.exec(`
          ...
          INSERT OR IGNORE INTO user_ratings_new (id, series_id, user_id, rating, created_at, updated_at)
            SELECT id, series_id, user_id, MIN(5, MAX(1, CAST(ROUND(rating / 2.0) AS INTEGER))), created_at, updated_at FROM user_ratings;
          DROP TABLE user_ratings;
          ALTER TABLE user_ratings_new RENAME TO user_ratings;
          CREATE INDEX IF NOT EXISTS idx_user_ratings_series ON user_ratings(series_id);
          CREATE INDEX IF NOT EXISTS idx_user_ratings_user ON user_ratings(user_id);
        `);
        // Seri ortalamalarını 1-5 skalasına göre yeniden hesapla
        const seriesIds = db.prepare('SELECT DISTINCT series_id FROM user_ratings').all();
        const updateAvg = db.prepare(`
          UPDATE series SET rating = (SELECT ROUND(AVG(rating), 2) FROM user_ratings WHERE series_id = ?) WHERE id = ?
        `);
        seriesIds.forEach(({ series_id }) => updateAvg.run(series_id, series_id));
      }
    } catch (e) { console.error('user_ratings 1-5 migration error:', e.message); }
